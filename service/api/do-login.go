...
func (rt *_router) doLogin(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	if err != nil {
		rt.baseLogger.WithError(err).Error("can't parse request body for DoLogin operation")
	...
	rt.baseLogger.Debugf(`received and parsed request for doLogin operation`)

	// Checking username validity
	regexpPattern := regexp.MustCompile(`^[a-zA-Z0-9_-]{3,16}$`)
	if !regexpPattern.MatchString(request.Username) {
		rt.baseLogger.Debugf(`username in request is invalid`)
		w.WriteHeader(http.StatusBadRequest)
		response := response.Problem{
			Title:  "Bad Request",
			Status: 400,
	...
	// Checking username validity
	rt.baseLogger.Debugf(`trying to retrive userId for user "%s"`, request.Username)
	userId, err := rt.db.GetUserId(request.Username)

	// Creating new user if username doesn't exists
	if errors.Is(err, sql.ErrNoRows) {
		rt.baseLogger.Debugf(`user "%s" does not exist, creating new user`, request.Username)
		newUserId, err := uuid.NewV4()
		if err != nil {
			rt.baseLogger.WithError(err).Error("can't generate a new user UUID")
		...
		if err != nil {
			rt.baseLogger.WithError(err).Error("can't insert a new user in database")
			w.WriteHeader(http.StatusInternalServerError)
			response := response.Problem{
				Title:  "Internal Server Error",
				Status: 500,
		...
		rt.baseLogger.WithError(err).Error("can't get the userId for username")
		w.WriteHeader(http.StatusInternalServerError)
		response := response.Problem{
			Title:  "Internal Server Error",
			Status: 500,
