...
func (db *appdbimpl) GetPhotosByUser(userId string, startDate string) ([]schema.Photo, error) {
	...
	rows, err := db.c.Query(
		`
		SELECT Photo.photoId, Photo.authorId, User.username, Photo.caption, Photo.dateTime, Photo.numberOfLikes, Photo.numberOfComments
		FROM Photo JOIN User
		ON Photo.authorId = User.userId
		WHERE Photo.authorId = ? AND Photo.dateTime < ?
